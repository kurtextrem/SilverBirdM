...
class NotificationManager {
	...
	onClick(nId) {
		...
		this.clearNotification(nId)
			...
			.catch(e => console.error('Couldnt clear', e) && e)
	}
	...
	async notify(tweet) {
		...
		const [buttons, contextMessage, imageUrl, iconUrl] = await Promise.all([
			...
			this.getBlobUrl(
				nId,
				tweet.user.profile_image_url_https.replace(/_(?:normal|big{2}er|mini)\.(png|jpe?g|gif)$/i, '.$1'),
		...
		return this.createNotification(nId, {
			...
			message: tweet.text.replace(/ht{2}ps?:\/{2}t\.co\/\w+\S/gi, '').replace(/^(?:@[^ ]+ )+/, ''), // replace all @'s - the new twitter feature
	...
	async normalize(tweet) {
		if (tweet.event !== undefined && tweet.event === 'quoted_tweet') tweet = tweet.target_object

		const oldScreenName = tweet.user.screen_name,
		...
		tweet.text = (tweet.full_text || tweet.text || '').trim()
		...
		tweet.text = tweet.text.replace(/\s+/g, ' ').replace(/\r?\n+/g, '\n')
		if (tweet.originalUrl === undefined) tweet.originalUrl = `https://twitter.com/${tweet.user.screen_name}/status/${tweet.id_str}`
	...
	async getButtomsFromURLs(nId, tweet) {
		...
		const match = tweet.entities.urls,
			arr = new Array(match.length),
			faviconUrl = 'https://www.google.com/s2/favicons?domain_url='

		for (let i = 0; i < match.length; ++i) {
			var url = typeof match[i] === 'object' ? match[i].expanded_url : match[i]
			arr[i] = {
				title: url,
