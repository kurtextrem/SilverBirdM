class NotificationManager {
    ...
    async __notify (tweet = { id_str: null }) {
        ...
        // create notification
        console.log(tweet)
        var buttons = this.__getButtomsFromURLs(tweet)
        try {
            const iconUrl = await this.__getIconUrl(nId, tweet.user.profile_image_url_https)
            const notifiedId = await this.__createNotification(nId, {
                ...
                message: tweet.text,
                contextMessage: (tweet.in_reply_to_user_id ? `In Reply To @${tweet.entities.user_mentions[0].name}` : ''),
                iconUrl,
                buttons
                // imageUrl
            })
