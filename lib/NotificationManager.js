...
class NotificationManager {
	...
	async notify(tweet) {
		...
		const [buttons, contextMessage, imageUrl, iconUrl] = await Promise.all([
			...
			this.getBlobUrl(
				nId,
				tweet.user.profile_image_url_https.replace(/_(?:normal|bigger|mini)\.(png|jpe?g|gif)$/i, '.$1'),
		...
		return this.createNotification(nId, {
			...
			message: tweet.text.replace(/(?:https?:\/\/t\.co\/\w+\S)/gi, '').replace(/^(?:@[^ ]+ )+/, ''), // replace all @'s - the new twitter feature
