...
class NotificationManager {
	...
	constructor() {
		...
		this.granted = false
		this.notificationTimeout = 6000
		this.currentlyActive = 0
		this.maxActive = 2
		chrome.runtime.getPlatformInfo(info => { if (info.os === 'mac') this.maxActive = 1 })

		// const i18n = chrome.i18n.getMessage
		this.constantStrings = chrome.i18n.getMessage /*{
		...
		window.setInterval(this.clearList.bind(this), 600000)
	}
	...
	setListener() {
		...
		notifications.onButtonClicked.addListener((nId, buttonIndex) => {
			console.log("clicked", nId, buttonIndex)
	...
	async dequeue() {
		if (!this.granted || this.queue.length === 0 || this.currentlyActive > this.maxActive) {
			console.warn("Queue empty / too many notifications", this.queue)
			return false
		}
	...
	async notify(tweet) {
		++this.currentlyActive // on erorr, we only increase, but don't decrease
	...
	createNotification(nId, option) {
		...
		return new Promise((resolve, reject) => {
			...
			chrome.notifications.create(nId, option, _nId => {
				if (chrome.runtime.lastError) return reject(chrome.rPuntime.lastError.message)

				console.log("notification", _nId)
