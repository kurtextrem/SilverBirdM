...
class NotificationManager {
	...
	async getBlobUrl(nId, url, fallbackUrl = this.defaultIcon) {
		...
		try {
			...
			throw new Error('Failed to fetch original icon, go to fallback')
		} catch (e) {
			if (fallbackUrl === '')
				return ''
	...
	async getImage(nId, tweet) {
		if (tweet.entities.media && tweet.entities.media.length)
			return await this.getBlobUrl(nId + '_image', tweet.entities.media[0].media_url_https, '')
		return ''
	}
