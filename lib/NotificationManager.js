...
class NotificationManager {
	...
	constructor() {
		...
		window.setInterval(() => this.dequeue(false), 4000)
	}
	...
	setListener() {
		...
		notifications.onPermissionLevelChanged.addListener(() => this.checkGranted())
		notifications.onClicked.addListener(this.onClick.bind(this))
		notifications.onClosed.addListener(() => this.dequeue())
	...
	addListForNotify(list) {
		if (Array.isArray(list)) this.queue.push(...list)
		else this.queue.push(list)
	}

	clearList() {
		this.queue.length = 0
		for (const entry in this.notified) {
			this.notified.set(entry, null)
		}
	}

	dequeue(intern) {
		if (!this.granted || (this.running && !intern)) return false
		if (this.queue.length === 0) return (this.running = false)

		this.running = true
		return this.notify(this.queue.shift())
			.then(nId => this.dequeue(true))
			.catch(e => {
	...
	createNotification(nId, option) {
		...
		return new Promise((resolve, reject) => {
			...
			chrome.notifications.create(nId, option, _nId => {
				...
				return window.setTimeout(function timeout() {
					resolve(_nId)
