...
class NotificationManager {
	...
	constructor() {
		...
		Object.defineProperties(this, {
			...
			avatarRegex: {
				value: /_(normal|bigger|mini)\.(png|jpe?g|gif)$/i
			}
		})
	...
	async __getBlobUrl(nId, url, fallbackUrl = this.defaultIcon) {
		if (!nId)
		...
		try {
			...
			if (response.ok) {
				...
				this.fetchedImg.set(nId, URL.createObjectURL(iconBlob))
				return this.fetchedImg.get(nId)
			}
			throw new TypeError('fail to fetch original icon, go to fallback')
		} catch (e) {
			if (fallbackUrl === '')
				return ''
			return await this.__getBlobUrl(nId, fallbackUrl)
		}
	}
	...
	async __notify(tweet = { id_str: null }) {
		...
		try {
			var [buttons, contextMessage, imageUrl, iconUrl] = await Promise.all([
				this.__getButtomsFromURLs(nId, tweet),
				this.__getContextMessage(tweet),
				this.__getImage(nId, tweet),
				this.__getBlobUrl(nId, tweet.user.profile_image_url_https.replace(this.avatarRegex, '.$2'), tweet.user.profile_image_url_https)
			])
	...
	async __createNotification(nId, option) {
	...
	async __getButtomsFromURLs(nId, tweet) {
		var arr = [],
			match = null
		...
		for (var i = 0; i < match.length; ++i) {
			...
			arr.push({
				title: url,
				iconUrl: await this.__getBlobUrl(nId + '_favicon' + i, 'https://www.google.com/s2/favicons?domain_url=' + url)
			})
		}
	...
	async __getImage(nId, tweet) {
		if (tweet.entities.media && tweet.entities.media.length)
			return await this.__getBlobUrl(nId + '_image', tweet.entities.media[0].media_url_https, '')
		return ''
	}
