...
TweetManager.prototype = {
	...
	showTweetsNotifications: function(tweetsToNotify = []) {
		if (tweetsToNotify.length === 0 || OptionsBackend.get('tweets_notification_style') === 'never') {
			return
		}
			...
			if (!success) {
				...
				const replaseFunc = data => {
					const text = this.isBlocked(data.user.id_str)
						...
						: this.isMuting(data.user.id_str) ? chrome.i18n.getMessage('n_muting_user') : ''
					if (text !== '') {
	...
	retrieveLists: function() {
		return Promise.all([this.retrieveOwnerships(), this.retrieveSubscriptions()])
			.then(() => {
	...
	retrieveTrendingTopics: function() {
		if (this.cachedTrendingTopics && Date.now() - new Date(this.cachedTrendingTopics.created_at).getTime() < 90 * 1000) {
			return
		}
