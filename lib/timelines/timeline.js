...
class TweetsTimeline {
  ...
  onStreamData(data) {
    if(!this._isExcludeStreamData(data)) {
  ...
  /* Private Methods */
  _isExcludeStreamData(data) {
    return !this.shouldListenStream
           || !data
           || !!data.delete
           || !!data.scrub_geo
           || !!data.limit
           || !!data.status_withheld
           || !!data.user_withheld
           || !!data.disconnect
           || !!data.warning
           || !!data.friends_str
  ...
  __syncTweetsReduceFunc(tweet, current, index) {
    ...
    if(current === tid) {
      ...
      if(!!this.newTweetsCache[index]) {
        if(!!this.newTweetsCache[index].timestamp_ms && !tweet.timestamp_ms) {
          tweet.timestamp_ms = this.newTweetsCache[index].timestamp_ms;
          tweet.silm_parsed_created_at = parseInt(tweet.timestamp_ms);
        }
        this.newTweetsCache[index] = tweet;
      } else if(!!this.tweetsCache[tweetsCacheIndex]) {
        if(!!this.tweetsCache[tweetsCacheIndex].timestamp_ms && !tweet.timestamp_ms) {
          tweet.timestamp_ms = this.tweetsCache[tweetsCacheIndex].timestamp_ms;
          tweet.silm_parsed_created_at = parseInt(tweet.timestamp_ms);
        }
  ...
  _isTweetBlocked(tweet) {
    ...
    if(this.manager.isBlocked(tweet.user.id_str)
    || (isRT && this.manager.isBlocked(tweet.retweeted_status.user.id_str))
    ) {
      return true;
    }
    if(this.template.excludeBlockedMuted
    && (this.manager.isMuting(tweet.user.id_str)
    || (isRT && this.manager.isMuting(tweet.retweeted_status.user.id_str)))
    ) {
      return true;
    }
