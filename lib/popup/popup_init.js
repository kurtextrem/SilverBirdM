...
function loadNewTweets() {
	...
	window.requestAnimationFrame(resetTab)
...
function bindEvents() {
	...
	$(window).on({
		...
		silmMessage(event) {
			...
			if (Boolean(event.detail) && Boolean(event.detail.type)) {
				switch (event.detail.type) {
					case 'createSearchTab':
						if (Boolean(event.detail.target) && event.detail.target.length > 0) {
							TimelineTab.addNewSearchTab(event.detail.target)
						}
						break
					case 'hideTweet':
						if (Boolean(event.detail.target) && event.detail.target.length > 0) {
							let removeTarget = document.querySelector(`[tweetid="${event.detail.target}"]`)
							if (removeTarget === undefined) break
						...
						break
					case 'showMessage':
						if (Boolean(event.detail.target) && Boolean(event.detail.target.message) && Boolean(event.detail.target.message) > 0) {
