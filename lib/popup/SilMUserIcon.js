;(function(customElements) {
	...
	const defaultProfileImageURL = 'https://abs.twimg.com/sticky/default_profile_images/default_profile.png'
	...
	function handleDecode(node) {
		node.shadowRoot.querySelector('img').remove()
	...
	function fetchImage(node, url = defaultProfileImageURL) {
		...
		if (img !== undefined && img.decode !== undefined) {
			...
			img
				.decode()
				.then(handleDecode.bind(img, node))
				.catch(handleError.bind(shadowIcon))
		} else {
			img = undefined
			shadowIcon.src = src
