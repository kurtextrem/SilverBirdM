const WorkList = class WorkList{
  ...
  static sendFailed(status) {
    ...
    const composer = document.querySelector("silm-composer");
    if(!(composer.getAttribute("composed") === "true" && composer.shadowRoot.querySelector("#tweet_text").value.length > 0)) {
      // For now let's just show the first enqueued message
      let topMessage = abortedQueue[0];
      Composer.initMessage(topMessage.message, topMessage.replyId, topMessage.replyUser, topMessage.quoteTweetUrl, topMessage.mediaIds);
    }
