const Composer = {
  ...
  initMessage: function(message, replyId, replyUser, quoteTweetUrl, mediaIds) {
    ...
    if(!!Composer.quoteTweetUrl) {
      document.querySelector("silm-composer").setAttribute("composed", "true");
      document.querySelector("silm-composer").setAttribute("quote", Composer.quoteTweetUrl);
    }
    Composer.mediaIds = mediaIds || new Map();
    for(var entry of Composer.mediaIds.entries()) {
      Composer.addMediaId(entry[0], entry[1], true);
    }
    document.querySelector("silm-composer").tweetText = message || '';
    document.querySelector("silm-composer").composed = "true";
    Composer.textareaChanged();
  },
  ...
  addUser: function (replies = []) {
    ...
    let currentVal = document.querySelector("silm-composer").tweetText;
    if(currentVal.length > 0 && currentVal[currentVal.length - 1] !== ' ') {
      currentVal += ' ';
    }
    document.querySelector("silm-composer").tweetText = `${currentVal}${replies.join(' ')}`;
  },
  ...
  reply: function (targetId, targetName) {
    ...
    const currentText = document.querySelector("silm-composer").tweetText;
    if(replies.size > 0) {
  ...
  message: function(targetName) {
    ...
    document.querySelector("silm-composer").composed = "true";
    document.querySelector("silm-composer").tweetText = `d ${targetName} `;
    Composer.textareaChanged();
  },

  textareaChanged: function () {
    var str = document.querySelector("silm-composer").tweetText || '';
...
//    if(document.getElementById('tweetit').getAttribute('disabled') == 'disabled') return;
    var message = document.querySelector("silm-composer").tweetText || '';
    ...
    Composer.textareaChanged();
    document.querySelector("silm-composer").composed = "false";
  },
  ...
  addMediaId: function(mediaId, dataURI, thumbnailOnly) {
    ...
    .on({
      "load.thumbnail": function(event) {
        ...
        document.querySelector("silm-composer").setAttribute("quote", "");
        Composer.textareaChanged();
      },
  ...
  resetComposerData: function(withSync) {
    document.querySelector("silm-composer").tweetText = '';
  ...
  syncComposerData: function() {
    ...
    tmCompose.saveMessage = document.querySelector("silm-composer").tweetText;
    ...
    Shortener.sync(document.querySelector("silm-composer").shadowRoot.querySelector("silm-shortener").shortenerText);
  },
