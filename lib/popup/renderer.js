const Renderer = {
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				ret.splice(
					indexStart,
					1,
					`<a href="${Renderer.URLS.BASE}${item.screen_name}" class="createUserActionMenu" data-user-id="${item.id_str}" data-user-name="${item.screen_name}">${ret[indexStart] ||
						''}`
				)
			...
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				if (searchRegexp.test(item.expanded_url)) {
					...
					if (!searchStrings.has('q')) {
						...
						const displayStrings = searchStrings.get('q').replace(Renderer.entitiesRegexp.displayStrings, `$1`)
					...
					spliceArray[0] = `<span><span class="glyphicon glyphicon-link"></span><a href="${item.url}" class="expandInReply" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" data-expand-in-reply-tweet="${escape(JSON.stringify(quotedTweetEntity))}" data-expand-in-reply-id="${tweetSpaceId}" title="${Renderer.constantStrings
						.expand_quote}">${item.display_url}</a></span>`
				} else {
					spliceArray[0] = `<a href="${item.url}" class="handleLink" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" title="${item.expanded_url}">${item.display_url}</a>`
				}
	...
	renderTweet(tweet, now, displayOptions) {
		...
		const templateId = tweet.silm_tweetTimelineId.replace(Renderer.entitiesRegexp.templateId, '')
		...
		if (!displayOptions.hiddenRetweetInfo && (Boolean(tweet.retweeted_status) || Boolean(tweet.current_user_retweet))) {
			...
			if (selfTweet || Boolean(tweet.current_user_retweet)) {
				...
				retweets_container += `<span class="inRetweet">${Renderer.buildIcon(tweet, false, { hiddenUserIcons: displayOptions.hiddenUserIcons, iconSize: 'retweeter' })}${Renderer
					.constantStrings.retweetedBy_prefix}<a href="${Renderer.URLS.BASE}${tweet.user.screen_name}" data-user-id="${tweet.user.id_str}" data-user-name="${tweet.user
					.screen_name}" class="createUserActionMenu">${tweet.user.screen_name}</a>${Renderer.constantStrings.retweetedBy_suffix}</span>`
			}
		...
		// icon_conteiner
		const icon_container = `<div class="icon_container">${Renderer.buildIcon(tweet, true, displayOptions)}</div>`

		// name_container
		const name_container = Renderer.buildName(tweet, true, displayOptions)

		// text_container
		const text_container = `<div class="text_container">${Renderer.parseEntities(tweet)}</div>`
		...
		if (!displayOptions.hiddenFooter) {
			...
			if (!displayOptions.hiddenDMInfo && templateId == TimelineTemplate.SENT_DMS) {
				footer_content += `<span class="dm_recipient">${Renderer.constantStrings.sentTo_prefix}<a href="#" data-user-id="${tweet.recipient.id_str}" data-user-name="${tweet
					.recipient.screen_name}" class="createUserActionMenu">${tweet.recipient.name}</a>${Renderer.constantStrings.sentTo_suffix}</span>`
			}
			// geo
	...
	buildGeo(tweet, isRetweet = true, displayOptions = { hiddenGeoInfo: false }) {
		...
		if (!displayOptions.hiddenGeoInfo && Boolean(geo)) {
			...
			const mapParam = [['center', `${latStr}`], ['zoom', '15'], ['size', '200x200'], ['maptype', 'roadmap'], ['markers', `size:small|${latStr}`], ['sensor', 'false']]
				.map(([q, v]) => `${q}=${v}`)
				.join('&')
	...
	buildReply(tweet, isRetweet = true, displayOptions = { hiddenReplyInfo: false }) {
		...
		if (!displayOptions.hiddenReplyInfo && Boolean(baseTweet.in_reply_to_status_id_str)) {
			...
			)}" data-expand-in-reply-id="${tweet.silm_tweetSpaceId}" href="#">${baseTweet.in_reply_to_screen_name}</a>${Renderer.constantStrings.inReply_suffix}</span>`
		}
		return ret
	},
	...
	buildReplyAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		let ret = ''
		if (!(isDM && isSelf)) {
			ret = `<span class="glyphicon glyphicon-reply new_actions_item action_reply" title="${Renderer.constantStrings.reply}" data-reply-target-url="${Renderer.buildTimestampUrl(
				tweet,
				isRetweet
			)}" data-reply-to-dm="${isDM}"></span>`
		}
		return ret
	},
	...
	buildDmAction(tweet, isRetweet = true, isDM = false) {
		...
		if (!isDM && enableDM) {
			ret = `<span class="glyphicon glyphicon-envelope new_actions_item action_message" title="${Renderer.constantStrings.directMessage}" data-message-target-name="${baseTweet.user
				.screen_name}"></span>`
		}
		return ret
	},
	...
	assemblyTweets(tweets, timelineId) {
		...
		$destination.html(renderdText)
		Renderer.applyObserve($destination[0])
		$destination.on('click', '.new_actions_item', event => {
