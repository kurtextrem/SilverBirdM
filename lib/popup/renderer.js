...
/*
      ...
      console.info(change.rootBounds);
      console.info(change.boundingClientRect);
      console.info(change.intersectionRatio);
      console.info(change.target);
*/
      if(change.intersectionRatio === 0) {
        change.target.querySelectorAll(".new_actions_item").forEach((el) => {
          if(el.hasOwnProperty("click")) {
            el.removeEventListener("click", el.click);
          }
        })
        change.target.querySelectorAll(".new_actions").forEach((el) => {
          if(el.hasOwnProperty("newActions")) {
            el.removeEventListener("mouseover", el.newActions.mouseover);
            el.removeEventListener("mouseout", el.newActions.mouseout);
          }
        })
      } else {
        change.target.querySelectorAll(".handleLink").forEach((el) => {
          Renderer.handleLinkEachFunc.bind(el)();
        })
        change.target.querySelectorAll(".tweet").forEach((el) => {
          if(!el.dataset.processed) {
            el.querySelectorAll(".createUserActionMenu").forEach((el) => {
              Renderer.createUserActionMenu.bind(el)();
            });
            el.querySelectorAll(".handleHashTag").forEach((el) => {
              Renderer.handleHashTagFunc.bind(el)();
            });
            el.querySelectorAll("[data-tooltip-do-expand]").forEach((el) => {
              Renderer.handleLinkForExpand.bind(el)();
            });
            if(!el.hasOwnProperty("newActions")) {
              Object.defineProperty(el, "newActions", {
                value: {
                  mouseover: (event, tweet = el) => {
                    const newActions = tweet.querySelector(".new_actions");
                    if(!newActions) {
                      return;
                    }
                    newActions.style.maxHeight = `${tweet.clientHeight}px`;
                    tweet.querySelectorAll(".new_actions_item").forEach((item) => {
                      if(!item.hasOwnProperty("click")) {
                        Object.defineProperty(item, "click", {
                          value: (event) => {
                        ...
                        item.addEventListener("click", item.click);
                      }
          ...
          el.addEventListener("mouseover", el.newActions.mouseover);
          el.addEventListener("mouseout", el.newActions.mouseout);
        });
        change.target.querySelectorAll(".expandInReply").forEach((el) => {
          Renderer.expandInReplyFunc.bind(el)();
        })
        change.target.querySelectorAll(".tooltip").forEach((el) => {
          $(el).tooltip({
            items: '.tooltip',
            show: {delay: 500},
            content: function() {
  ...
  applyUnobserve: () => {
    document.querySelectorAll(".tweet_space").forEach((el) => {
      if(el.hasOwnProperty("observed") && el.observed === true) {
        document.querySelectorAll(".tweet_space").forEach((el) => {
  ...
  decideBaseTweet: function(tweet, isRetweet = true, prop = null) {
    // work around: broken retweet from Streaming API
    if(tweet.hasOwnProperty("from") && tweet.from === "stream") {
      if(!!tweet.retweeted_status && !!tweet.text) {
        const delimiterIndex = tweet.text.indexOf(": ");
        if(delimiterIndex > 0) {
          tweet.retweeted_status.text = tweet.text.substr(delimiterIndex + 2);
        }
      }
    }
