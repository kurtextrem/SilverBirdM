const Renderer = {
	...
	io: new IntersectionObserver(
		changes => {
			for (let i = 0; i < changes.length; ++i) {
				...
				let dataCreatedAt = target.querySelectorAll('[data-created-at]:not([data-filled])')
				if (dataCreatedAt[0] !== undefined) {
					let now = Date.now()
					requestAnimationFrame(() => {
						for (let i = 0; i < dataCreatedAt.length; ++i) {
							let data = dataCreatedAt[i]
							data.textContent = Renderer.getTimestampText(new Date(data.dataset.createdAt), now)
						}
					})
				}

				requestIdleCallback(() => {
					let handleLinks = target.querySelectorAll('.handleLink')
					if (handleLinks[0] !== undefined) {
						handleLinks.forEach(Renderer.handleLinkEachFunc)
					}
					let createUserActionMenus = target.querySelectorAll('.createUserActionMenu')
					if (createUserActionMenus[0] !== undefined) {
						createUserActionMenus.forEach(el => {
							Renderer.createUserActionMenu.bind(el)()
						})
					}
					let handleHashTags = target.querySelectorAll('.handleHashTag')
					if (handleHashTags[0] !== undefined) {
						handleHashTags.forEach(Renderer.handleHashTagFunc)
					}
					let expandInReplies = target.querySelectorAll('.expandInReply')
	...
	entitiesRegexp: {
		quoteTweet: /^https?:\/\/(mobile\.)?twitter.com\/[\w\d_]{1,15}\/statuse?s?\/(\d+)\D*?/i,
		replyTargetUrl: /^https?:\/\/(mobile\.)?twitter.com\/([\w\d_]{1,15})\/statuse?s?\/(\d+)\D*?/i,
	...
	parseUserMentionsEntity(texts = [], entity = []) {
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				let [indexStart, indexEnd] = item.indices
				// const suspiciousTarget = ret.slice(indexStart, indexEnd).join('')
				ret.splice(
					indexStart,
					1,
					`<a href="${Renderer.URLS
						.BASE}${item.screen_name}" class="createUserActionMenu" data-user-id="${item.id_str}" data-user-name="${item.screen_name}">${ret[indexStart] || ''}`
				)
	...
	parseUrlsEntity(texts = [], entity = [], tweetSpaceId = '', quotedStatus = { id_str: null }) {
		const ret = texts
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				let [indexStart, indexEnd] = item.indices
				// const suspiciousTarget = ret.slice(indexStart, indexEnd).join('')
				let spliceLength = indexEnd - indexStart
				let spliceArray = new Array(spliceLength)
				spliceArray[0] = ''
				if (searchRegexp.test(item.expanded_url)) {
					...
					let searchStrings = new URLSearchParams(searchRegexp.exec(decodedUrl)[1] || '')
					if (!searchStrings.has('q')) {
						spliceArray[0] = `<a href="${item.url}" class="handleLink" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
							''}" title="${item.expanded_url}">${item.display_url}</a>`
					} else {
						let displayStrings = searchStrings.get('q').replace(Renderer.entitiesRegexp.displayStrings, `$1`)
					...
					spliceArray[0] = `<span><span class="glyphicon glyphicon-link"></span><a href="${item.url}" class="expandInReply" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" data-expand-in-reply-tweet="${escape(JSON.stringify(quotedTweetEntity))}" data-expand-in-reply-id="${tweetSpaceId}" title="${Renderer
						.constantStrings.expand_quote}">${item.display_url}</a></span>`
				} else {
					spliceArray[0] = `<a href="${item.url}" class="handleLink" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" title="${item.expanded_url}">${item.display_url}</a>`
				}
	...
	parseMediaEntity(texts = [], entity = [], extendedEntities = null) {
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				let spliceLength = indexEnd - indexStart
				let spliceArray = new Array(spliceLength) //.fill('')
				spliceArray[0] = ''
				let extendedAttribute = ''
				let suffixContent = ''
				if (Boolean(extendedEntities) && Array.isArray(extendedEntities.media) && extendedEntities.media.length > 1) {
					...
					suffixContent = ` ${extendedEntities.media
						.map(function(value, index) {
							return `<a href="${value.url}" class="handleLink" data-handle-link-base="${value.url}" data-handle-link-expanded="${value.expanded_url}" data-handle-link-media="${value.media_url_https}" title="${value.expanded_url}">[${index +
								1}]</a>`
						})
						.join(' ')}`
				} else {
	...
	renderTweet(tweet, now, displayOptions) {
		...
		let selfTweet = tweet.user.id_str === tweetManager.twitterBackend.userId
		if (selfTweet && !tweetManager.isRetweeted(tweet) && Boolean(tweet.retweeted_status)) {
		...
		let templateId = tweet.silm_tweetTimelineId.replace(Renderer.entitiesRegexp.templateId, '')
		let hereIsDM = templateId === TimelineTemplate.RECEIVED_DMS || templateId === TimelineTemplate.SENT_DMS || false
		let isDummy = Boolean(tweet.is_dummy) || false

		// retweets_container
		let retweets_container = ''
		if (!displayOptions.hiddenRetweetInfo && (Boolean(tweet.retweeted_status) || Boolean(tweet.current_user_retweet))) {
			...
			if (selfTweet || Boolean(tweet.current_user_retweet)) {
				...
				retweets_container += `<span class="inRetweet">${Renderer.buildIcon(tweet, false, {
					hiddenUserIcons: displayOptions.hiddenUserIcons,
					iconSize: 'retweeter',
				})}${Renderer.constantStrings.retweetedBy_prefix}<a href="${Renderer.URLS.BASE}${tweet.user.screen_name}" data-user-id="${tweet.user
					.id_str}" data-user-name="${tweet.user.screen_name}" class="createUserActionMenu">${tweet.user.screen_name}</a>${Renderer.constantStrings
					.retweetedBy_suffix}</span>`
			}
		...
		if (!displayOptions.hiddenFooter) {
			...
			if (!displayOptions.hiddenDMInfo && templateId === TimelineTemplate.SENT_DMS) {
				footer_content += `<span class="dm_recipient">${Renderer.constantStrings.sentTo_prefix}<a href="#" data-user-id="${tweet.recipient
					.id_str}" data-user-name="${tweet.recipient.screen_name}" class="createUserActionMenu">${tweet.recipient.name}</a>${Renderer.constantStrings
					.sentTo_suffix}</span>`
			}
			// geo
			...
			if (!displayOptions.hiddenListInfo && templateId === TimelineTemplate.LISTS && tweetManager.currentTimelineId !== tweet.silm_tweetTimelineId) {
				...
				if (list) {
					...
					footer_content += `<span class="from_list">(${Renderer.constantStrings
						.footer_list}: <a class="handleLink" data-handle-link-noexpand="true" data-handle-link-base="${Renderer.URLS
						.BASE}${linkPath}" href="#" title="@${linkPath}">${list.name}</a>)</span>`
				}
			}
		...
		if (displayOptions.useColors) {
			let color = displayOptions.colors[`${hereIsDM ? 'dms' : templateId}_tweets_color`]
	...
	buildTimestamp(tweet, now = Date.now(), isRetweet, withAnchor, displayOptions = { hiddenTimestamp: false }) {
		let ret = ''
		if (!displayOptions.hiddenTimestamp) {
			let baseTime = Renderer.decideBaseTweet(tweet, isRetweet, 'created_at')
			let processedTime = ''
			if (withAnchor) {
	...
	buildTimestampUrl(tweet, isRetweet = true) {
		let baseTweet = Renderer.decideBaseTweet(tweet, isRetweet)
		return `${Renderer.URLS.BASE}${baseTweet.user.screen_name}/status/${baseTweet.id_str}`
	},

	buildName(tweet, isRetweet = true, displayOptions = { displaySimpleName: false, nameAttribute: 'both' }) {
		let user = Renderer.decideBaseTweet(tweet, isRetweet, 'user')
		let usn = user.screen_name
		let uid = user.id_str
		...
		if (displayOptions.nameAttribute == 'both') {
			let bothContent = !displayOptions.displaySimpleName ? `</div><div class="secondary_name">` : ''
		...
		let userVerified =
			Boolean(user.verified) && !displayOptions.displaySimpleName
				? `<span class="glyphicon glyphicon-check verifiedAccount" title="${Renderer.constantStrings.verified_account}"></span>`
				: ''
		let userProtected =
			Boolean(user.protected) && !displayOptions.displaySimpleName
				? `<span class="glyphicon glyphicon-lock protectedAccount" title="${Renderer.constantStrings.protected_account}"></span>`
				: ''
		return `<div class="name_container"><div class="primary_name">${ret}</div>${userVerified}${userProtected}</div>`
	},

	buildFromApp(tweet, isRetweet = true, displayOptions = { hiddenClientName: false }) {
		if (displayOptions.hiddenClientName) return ''

		let source = Renderer.decideBaseTweet(tweet, isRetweet, 'source')
		let ret = ''
		if (source) {
			source = source.replace(Renderer.entitiesRegexp.source, `class="handleLink" href="#" data-handle-link-noexpand="true" data-handle-link-base=`)
			ret = `<span class="from_app">${Renderer.constantStrings.fromApp_prefix}${source}${Renderer.constantStrings.fromApp_suffix}</span>`
		}
		return ret
	},

	buildIcon(tweet, isRetweet = true, displayOptions = { hiddenUserIcons: false, iconSize: 'icon_normal' }) {
		let ret = ''
		if (!displayOptions.hiddenUserIcons) {
			let user = Renderer.decideBaseTweet(tweet, isRetweet, 'user')
			let [iconSize, iconStyle] = Renderer.iconStyleTable.get(displayOptions.iconSize) || ['_normal.', 'icon_normal']
			ret = `
        <silm-usericon
          class="profile ${iconStyle}"
          src="${user.profile_image_url_https.replace(Renderer.entitiesRegexp.userIcon, iconSize)}"
        ></silm-usericon>
      `
		}
		return ret
	...
	buildRetweetCounts(tweet, isRetweet = true, displayOptions = { hiddenRetweetCount: false }) {
		let ret = ''
	...
	buildLikeCounts(tweet, isRetweet = true, displayOptions = { hiddenLikeCount: false }) {
		let ret = ''
	...
	buildReply(tweet, isRetweet = true, displayOptions = { hiddenReplyInfo: false }) {
		let ret = ''
		const baseTweet = Renderer.decideBaseTweet(tweet, isRetweet)
		if (!displayOptions.hiddenReplyInfo && Boolean(baseTweet.in_reply_to_status_id_str)) {
			ret = `<span class="inReply">${Renderer.constantStrings.inReply_prefix}<a class="expandInReply" data-expand-in-reply-tweet="${escape(
				JSON.stringify({ in_reply_to_status_id_str: baseTweet.in_reply_to_status_id_str })
			)}" data-expand-in-reply-id="${tweet.silm_tweetSpaceId}" href="#">${baseTweet.in_reply_to_screen_name}</a>${Renderer.constantStrings
				.inReply_suffix}</span>`
		}
		return ret
	},
	...
	buildReplyAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		let ret = ''
		if (!(isDM && isSelf)) {
			ret = `<span class="glyphicon glyphicon-reply new_actions_item action_reply" title="${Renderer.constantStrings
				.reply}" data-reply-target-url="${Renderer.buildTimestampUrl(tweet, isRetweet)}" data-reply-to-dm="${isDM}"></span>`
		}
		return ret
	},
	...
	buildRetweetAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		...
		if (!isDM && !(baseTweet.user.protected && !isSelf) && !baseTweet.retweeted) {
			ret += `<span class="glyphicon glyphicon-retweet new_actions_item action_retweet" title="${Renderer.constantStrings
				.retweet}" data-retweet-target-id="${baseTweet.id_str}"></span>`
		}
		if (!isDM && !(baseTweet.user.protected && !isSelf)) {
			ret += `<span class="glyphicon glyphicon-comment new_actions_item action_quote" title="${Renderer.constantStrings
				.quoteTweet}" data-quote-tweet-url="${Renderer.buildTimestampUrl(tweet, isRetweet)}"></span>`
		}
		return ret
	},
	...
	buildDeleteAction(tweet, isDM = false, isSelf = false) {
		...
		if (Boolean(isDM) || Boolean(isSelf)) {
			ret = `<span class="glyphicon glyphicon-trash new_actions_item action_delete_tweet" title="${Renderer.constantStrings
				.deleteTweet}" data-delete-target-id="${tweet.id_str}" data-timeline-id="${tweet.silm_tweetTimelineId}"></span>`
		}
		return ret
	},
	...
	buildDeleteRetweetAction(tweet, isSelf = true) {
		let ret = ''
		if ((Boolean(tweet.retweeted_status) && Boolean(isSelf)) || Boolean(tweet.current_user_retweet)) {
			const tweetId = tweet.current_user_retweet ? tweet.current_user_retweet.id_str : tweet.id_str
			ret = `<span class="glyphicon glyphicon-remove-circle new_actions_item action_cancel_retweet" title="${Renderer.constantStrings
				.deleteRT}" data-delete-target-id="${tweetId}" data-timeline-id="${tweet.silm_tweetTimelineId}"></span>`
		}
		return ret
	},
	...
	buildDmAction(tweet, isRetweet = true, isDM = false) {
		let ret = ''
		const baseTweet = Renderer.decideBaseTweet(tweet, isRetweet)
		const enableDM = baseTweet.user.allow_dms_from === 'everyone' || Boolean(tweetManager.isFollowedBy(baseTweet.user.id_str))
		if (!isDM && enableDM) {
			ret = `<span class="glyphicon glyphicon-envelope new_actions_item action_message" title="${Renderer.constantStrings
				.directMessage}" data-message-target-name="${baseTweet.user.screen_name}"></span>`
		}
		return ret
	},
		...
		switch (true) {
			...
			case cl.contains('action_reply'):
				if (event.target.dataset.replyToDm !== 'true') {
					...
					let targetName = Renderer.entitiesRegexp.replyTargetUrl.exec(event.target.dataset.replyTargetUrl)[2]
	...
	openTab(tabUrl, event) {
		...
		if (!/^https?:\/\//.test(tabUrl) && !/^chrome-extension:\/\//.test(tabUrl)) {
			tabUrl = 'http://' + tabUrl
		}
		tabUrl.replace(/\W.*$/, '')
		if (!background) {
