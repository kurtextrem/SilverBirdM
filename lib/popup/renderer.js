const Renderer = {
			...
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				ret.splice(
					...
					`<a href="${Renderer.URLS
						.BASE}${item.screen_name}" class="createUserActionMenu" data-user-id="${item.id_str}" data-user-name="${item.screen_name}">${ret[
						indexStart
					] || ''}`
				)
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				ret.splice(
					indexStart,
					1,
					`<a href="${item.url}" class="handleHashTag" data-handle-hash-tag="#${item.text}">${ret[indexStart] || ''}`
				)
				ret.splice(indexEnd - 1, 1, `${ret[indexEnd - 1] || ''}</a>`)
			}
		})
		...
		entity.forEach(item => {
			if (Boolean(item.indices) || Array.isArray(item.indices)) {
				...
				if (searchRegexp.test(item.expanded_url)) {
					...
					spliceArray[0] = `<span class="inlineLink"><span class="material-icons">link</span><a href="${item.url}" class="expandInReply" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" data-expand-in-reply-tweet="${escape(
						JSON.stringify(quotedTweetEntity)
					)}" data-expand-in-reply-id="${tweetSpaceId}" title="${Renderer.constantStrings.expand_quote}">${item.display_url}</a></span>`
				} else {
					spliceArray[0] = `<a href="${item.url}" class="handleLink" data-handle-link-base="${item.url}" data-handle-link-expanded="${item.expanded_url}" data-handle-link-media="${item.media_url_https ||
						''}" title="${item.expanded_url}">${item.display_url}</a>`
				}
		...
		if (!displayOptions.hiddenRetweetInfo && (Boolean(tweet.retweeted_status) || Boolean(tweet.current_user_retweet))) {
			...
			if (selfTweet || Boolean(tweet.current_user_retweet)) {
				...
				retweets_container += `<span class="inRetweet">${Renderer.buildIcon(tweet, false, {
					...
					.id_str}" data-user-name="${tweet.user.screen_name}" class="createUserActionMenu">${tweet.user.screen_name}</a>${Renderer
					.constantStrings.retweetedBy_suffix}</span>`
			}
		...
		if (!displayOptions.hiddenFooter) {
			...
			if (!displayOptions.hiddenDMInfo && templateId === TimelineTemplate.SENT_DMS) {
				footer_content += `<span class="dm_recipient">${Renderer.constantStrings.sentTo_prefix}<a href="#" data-user-id="${tweet.recipient
					.id_str}" data-user-name="${tweet.recipient.screen_name}" class="createUserActionMenu">${tweet.recipient.name}</a>${Renderer
					.constantStrings.sentTo_suffix}</span>`
			}
			// geo
	...
	buildName(tweet, isRetweet = true, displayOptions = { displaySimpleName: false, nameAttribute: 'both' }) {
		...
		const userVerified =
			!!user.verified && !displayOptions.displaySimpleName
				? `<span class="material-icons verifiedAccount" title="${Renderer.constantStrings.verified_account}">verified_user</span>`
				: ''
		const userProtected =
			!!user.protected && !displayOptions.displaySimpleName
				? `<span class="material-icons protectedAccount" title="${Renderer.constantStrings.protected_account}">lock</span>`
				: ''
	...
	buildFromApp(tweet, isRetweet = true, displayOptions = { hiddenClientName: false }) {
		...
		const source = Renderer.decideBaseTweet(tweet, isRetweet, 'source')
		let ret = ''
		if (source) {
	...
	buildReplyAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		let ret = ''
		if (!(isDM && isSelf)) {
			ret = `<span class="material-icons new_actions_item action_reply" title="${Renderer.constantStrings
				.reply}" data-reply-target-url="${Renderer.buildTimestampUrl(tweet, isRetweet)}" data-reply-to-dm="${isDM}">reply</span>`
		}
		return ret
	},
	...
	buildRetweetAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		...
		if (!isDM && !(baseTweet.user.protected && !isSelf) && !baseTweet.retweeted) {
			ret += `<span class="material-icons new_actions_item action_retweet" title="${Renderer.constantStrings
				.retweet}" data-retweet-target-id="${baseTweet.id_str}">repeat</span>`
		}
		if (!isDM && !(baseTweet.user.protected && !isSelf)) {
			ret += `<span class="material-icons new_actions_item action_quote" title="${Renderer.constantStrings
				.quoteTweet}" data-quote-tweet-url="${Renderer.buildTimestampUrl(tweet, isRetweet)}">comment</span>`
		}
		return ret
	},
	...
	buildDeleteAction(tweet, isDM = false, isSelf = false) {
		...
		if (Boolean(isDM) || Boolean(isSelf)) {
			ret = `<span class="material-icons new_actions_item action_delete_tweet" title="${Renderer.constantStrings
				.deleteTweet}" data-delete-target-id="${tweet.id_str}" data-timeline-id="${tweet.silm_tweetTimelineId}">delete</span>`
		}
		return ret
	},
	...
	buildDeleteRetweetAction(tweet, isSelf = true) {
		...
		if ((Boolean(tweet.retweeted_status) && Boolean(isSelf)) || Boolean(tweet.current_user_retweet)) {
			...
			ret = `<span class="material-icons new_actions_item action_cancel_retweet" title="${Renderer.constantStrings
				.deleteRT}" data-delete-target-id="${tweetId}" data-timeline-id="${tweet.silm_tweetTimelineId}">cancel</span>`
		}
		return ret
	},
	...
	buildDmAction(tweet, isRetweet = true, isDM = false) {
		...
		if (!isDM && enableDM) {
			ret = `<span class="material-icons new_actions_item action_message" title="${Renderer.constantStrings
				.directMessage}" data-message-target-name="${baseTweet.user.screen_name}">mail_outline</span>`
		}
		return ret
	},
