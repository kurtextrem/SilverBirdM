	...
	io: new IntersectionObserver(
		changes => {
			for (let i = 0; i < changes.length; ++i) {
				...
				const dataCreatedAt = target.querySelectorAll('[data-created-at]:not([data-filled])')
...
const item = entity[i]
	...
	decideBaseTweet(tweet, isRetweet = true, prop = null) {
		const baseTweet = isRetweet && Boolean(tweet.retweeted_status) ? tweet.retweeted_status : tweet
		if (Boolean(prop)) {
