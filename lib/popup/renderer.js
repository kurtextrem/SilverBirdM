var Renderer = {
  ...
  parseEntities: function(tweet, isRetweet = true) {
    ...
    for(let i in baseTweet.entities) {
      let entry = baseTweet.entities[i];
      switch(i) {
        case "user_mentions":
          ret = Renderer.parseUserMentionsEntity(ret, entry);
          break;
        case "hashtags":
          ret = Renderer.parseHashtagsEntity(ret, entry);
          break;
        case "urls":
          ret = Renderer.parseUrlsEntity(ret, entry, tweet.silm_tweetSpaceId, baseTweet.quoted_status);
          break;
        case "media":
          ret = Renderer.parseMediaEntity(ret, entry, baseTweet.extended_entities);
          break;
        case "symbols":
        //  ret = Renderer.parseSymbolsEntity(ret, entry);
          break;
        default:
