const Renderer = {
  ...
  parseUserMentionsEntity: function(texts = [], entity = []) {
    ...
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        ...
        if(!/^[@＠]/.test(ret.slice(indexStart, indexEnd).join(""))) {
          console.info(`broken item.screen_name: %s`, ret.slice(indexStart, indexEnd).join(""));
        }
  ...
  parseHashtagsEntity: function(texts = [], entity = []) {
    ...
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        if(!/^[#＃]/.test(ret.slice(indexStart, indexEnd).join(""))) {
          console.info(`broken item.text: %s`, ret.slice(indexStart, indexEnd).join(""));
        }
  ...
  parseUrlsEntity: function(texts = [], entity = [], tweetSpaceId = "", quotedStatus = {id_str: null}) {
    ...
    entity.forEach((item) => {
      if(!!item.indices || Array.isArray(item.indices)) {
        const [indexStart, indexEnd] = item.indices;
        if(!/^https?/.test(ret.slice(indexStart, indexEnd).join(""))) {
          console.info(`broken item.indices: %s`, ret.slice(indexStart, indexEnd).join(""));
