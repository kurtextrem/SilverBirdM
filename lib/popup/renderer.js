	...
	io: new IntersectionObserver(
		changes => {
			...
			for (let i = 0; i < changes.length; ++i) {
				...
				if (changes[i].intersectionRatio < 0.01) {
					if (target.idleId) {
					...
					if (dataCreatedAt[0] !== undefined) {
						requestAnimationFrame(() => {
							for (var i = 0; i < dataCreatedAt.length; i++) {
								if (dataCreatedAt[i].textContent !== '') continue
								dataCreatedAt[i].textContent = Renderer.getTimestampText(new Date(dataCreatedAt[i].dataset.createdAt), now)
							}
						})
					}
				}
