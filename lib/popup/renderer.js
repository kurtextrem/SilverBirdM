const Renderer = {
	...
	renderTweet(tweet, now, displayOptions) {
		...
		if (!displayOptions.hiddenRetweetInfo && (Boolean(tweet.retweeted_status) || Boolean(tweet.current_user_retweet))) {
			...
			if (selfTweet || Boolean(tweet.current_user_retweet)) {
				...
				retweets_container += `<span class="inRetweet">${Renderer.buildIcon(tweet, false, { hiddenUserIcons: displayOptions.hiddenUserIcons, iconSize: 'retweeter' })}${Renderer.constantStrings
					.retweetedBy_prefix}<a href="${Renderer.URLS.BASE}${tweet.user.screen_name}" data-user-id="${tweet.user.id_str}" data-user-name="${tweet.user
		...
		if (!displayOptions.hiddenFooter) {
			...
			if (!displayOptions.hiddenDMInfo && templateId == TimelineTemplate.SENT_DMS) {
				footer_content += `<span class="dm_recipient">${Renderer.constantStrings.sentTo_prefix}<a href="#" data-user-id="${tweet.recipient.id_str}" data-user-name="${tweet.recipient
					.screen_name}" class="createUserActionMenu">${tweet.recipient.name}</a>${Renderer.constantStrings.sentTo_suffix}</span>`
			}
			// geo
			...
			if (!displayOptions.hiddenListInfo && templateId == TimelineTemplate.LISTS && tweetManager.currentTimelineId !== tweet.silm_tweetTimelineId) {
				...
				if (Boolean(list)) {
					...
					footer_content += `<span class="from_list">(${Renderer.constantStrings.footer_list}: <a class="handleLink" data-handle-link-noexpand="true" data-handle-link-base="${Renderer.URLS
	...
	buildName(tweet, isRetweet = true, displayOptions = { displaySimpleName: false, nameAttribute: 'both' }) {
		...
		const userVerified =
			Boolean(user.verified) && !displayOptions.displaySimpleName ? `<span class="glyphicon glyphicon-check verifiedAccount" title="${Renderer.constantStrings.verified_account}"></span>` : ''
		const userProtected =
			Boolean(user.protected) && !displayOptions.displaySimpleName ? `<span class="glyphicon glyphicon-lock protectedAccount" title="${Renderer.constantStrings.protected_account}"></span>` : ''
	...
	buildRetweetAction(tweet, isRetweet = true, isDM = false, isSelf = false) {
		...
		if (!isDM && !(baseTweet.user.protected && !isSelf)) {
			ret += `<span class="glyphicon glyphicon-comment new_actions_item action_quote" title="${Renderer.constantStrings.quoteTweet}" data-quote-tweet-url="${Renderer.buildTimestampUrl(
				tweet,
				isRetweet
			)}"></span>`
		}
	...
	buildDeleteRetweetAction(tweet, isSelf = true) {
		...
		if ((Boolean(tweet.retweeted_status) && Boolean(isSelf)) || Boolean(tweet.current_user_retweet)) {
			const tweetId = Boolean(tweet.current_user_retweet) ? tweet.current_user_retweet.id_str : tweet.id_str
