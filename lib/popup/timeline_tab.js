var TimelineTab = {
	...
	addNewTab(templateId, automaticallyAdded) {
		...
		for (var i = 0; i < createdTimelines.length; ++i) {
			...
			switch (templateId) {
				...
				case TimelineTemplate.LISTS:
					TimelineTab.addTab(
						timeline.timelineId,
						`<select id="${timeline.timelineId}-selector" data-timeline-id="${timeline.timelineId}"></select>`
					)
	...
	select(timelineId) {
		...
		tweetManager.currentTimelineId = timelineId
		TimelineTab.handleScroll(tweetManager.previousTimelineId, false)
		Renderer.preClearance()
		prepareTimelines()

		TimelineTab.handleScroll(timelineId, true)
		loadTimeline()
		// requestIdleCallback(() => ui.oldPanel.find('.inner_timeline').empty())
		window.setTimeout(() => $('#timeline-' + timelineId).css('display', 'block'), 1000)
	...
	wheel(e) {
		return Paginator.needsMore && e.preventDefault()
	},
	...
	handleScroll(timelineId = tweetManager.currentTimelineId, doHandle = true) {
		...
		if (doHandle) {
			...
			target.addEventListener('wheel', TimelineTab.wheel)
		} else if (target._handler !== undefined) {
			target.removeEventListener('scroll', target._handler)
			target.removeEventListener('wheel', TimelineTab.wheel)
