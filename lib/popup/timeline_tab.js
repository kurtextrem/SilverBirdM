var TimelineTab = {
  init: function() {
    $("#tabs").tabs({
      ...
      activate: function(event, ui) {
        ...
        ui.newPanel.find('.inner_timeline').scrollTop(tweetManager.getCurrentTimeline().currentScroll || 0);
      }
    });
  },
  ...
  addTab: function(timelineId, tabName, pos) {
    ...
    tabDiv.tabs('refresh');
    $(`#timeline-${timelineId} .inner_timeline`).on("scroll", function(event) {
      const timeline = tweetManager.getTimeline(timelineId);
      const threshold = 50;
      const scrolledValue = event.target.scrollTop;
      const scrollAmount = timeline.currentScroll - scrolledValue;
      timeline.currentScroll = scrolledValue;
      if(scrollAmount < 0 && (event.target.scrollHeight - event.target.clientHeight - scrolledValue) < threshold) {
