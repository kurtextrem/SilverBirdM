...
class TwitterRequestBase {
  ...
  async generateSignature(request) {
    const keyBinary = await crypto.subtle.importKey("raw", this.signKey, {
      name: "HMAC",
      hash: {name: "SHA-1"}
    }, false, ["sign"]);
    const signatureBinary = await crypto.subtle.sign({
      name: "HMAC",
      hash: "SHA-1"
    }, keyBinary, this.generateBaseStrings(request));
    const signatureStrings = btoa(new Uint8Array(signatureBinary).reduce((prev, current) => {
      return prev + String.fromCharCode(current);
    }, ""));
    request.query.append("oauth_signature", signatureStrings);
    return request;
  }
  fetchRequest() {
...
class TwitterUpload extends TwitterRequest {
  ...
  async uploadMedia(file, isDM = false, callback = console.log) {
    this.callback = callback;
    if(!file || !file.size || !file.type) {
      this.callback("missing file");
    }
    try {
      const json = await this.uploadMediaInit(file, isDM);
      if(!json) {
      ...
      let result = await this.uploadMediaFinalize(mediaId);
      while(true) {
        if(!!result.prosessing_info) {
        ...
        if(result.processing_info.state === "succeeded") {
          this.callback(result);
          return result;
        } else if(result.processing_info.state === "failed") {
          console.warn(result);
          throw new TypeError(result.processing_info.error.name);
        } else if(result.processing_info.state === "pending" || result.processing_info.state === "in_progress") {
          if(result.processing_info.check_after_secs > 0) {
            await this.wait(result.processing_info.check_after_secs);
          }
          result = await this.uploadMediaStatus(mediaId);
        } else {
          throw new TypeError(`uncaught result: %o`, result);
        }
      }
    } catch(e) {
      console.warn(e);
  ...
  async uploadMediaInit(file, isDM = false) {
    const categoryPrefix = (isDM)? "dm": "tweet";
    ...
    switch(true) {
      ...
      case /image\/gif/i.test(file.type):
        mediaType = "gif";
        break;
      case /image\/(:?jpeg|png|webp?)/i.test(file.type):
        // no behavior
        break;
      default:
        throw new TypeError("uncaught file type");
        break;
    }
    params.set("media_category", `${categoryPrefix}_${mediaType}`);
    try {
      const signedRequest = await this.generateSignature({
        method: "POST",
