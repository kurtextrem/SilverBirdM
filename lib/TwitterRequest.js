...
class UserStream extends TwitterRequest {
  ...
  readStream() {
    ...
    return this.streamReader.read().then((result) => {
      ...
      if(decodedData !== "\r\n") {
        this.processStream(decodedData);
      }
      if(!result.done) {
    ...
    try {
      ...
      if(/^[\[\{]/.test(stream)) {
        ...
        this.buffer = this.buffer.map((entry) => {
          const joind = entry + stream;
          try {
            this.dispatch(JSON.parse(joind));
            return undefined;
          } catch(e) {
            if(joind.length > 1024 * 32) {
              console.log("UserStream.processStream.bufferOverflow: %s", joined.length);
              return undefined;
            }
            return joind;
          }
